...
ant_geo_ilm = read_field(
    "D:\\tvg_toolkit\\fingerprint_approach\\测试sle\\ant_001_rotation.geo", 60
)
ant_upl_ilm = read_field(
    "D:\\tvg_toolkit\\fingerprint_approach\\测试sle\\ant_001_rotation.upl", 60
)
...
oc = np.loadtxt("D:\\tvg_toolkit\\tvg_toolkit\\data\\oc_func_100km.txt")[:, 2].reshape(
    180, 360
)
...
p = ax1.pcolormesh(
    lon_x, lat_y, ant_upl - ant_upl_norot, transform=ccrs.PlateCarree(), cmap="jet"
)
...
ax2.pcolormesh(
    lon_x, lat_y, upl - upl_norot, transform=ccrs.PlateCarree(), cmap="jet", norm=p.norm
)
