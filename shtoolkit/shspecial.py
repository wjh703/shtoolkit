...
def calc_rot(L_cilm_real: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    J13 = a**4 * np.pi * (32/15)**.5 * L_cilm_complex[0, 2, 1]
    J23 = -a**4 * np.pi * (32/15)**.5 * L_cilm_complex[1, 2, 1]
    J33 = -a**4 * np.pi * (8/3/5**.5) * L_cilm_complex[0, 2, 0]
    ...
    La_cilm_complex[0, 2, 0] = a**2 * omega**2 / (6*5**.5) * (m1**2+m2**2-2*m3**2-4*m3)
    La_cilm_complex[0, 2, 1] = a**2 * omega**2 / 30**.5 * (m1*(1+m3))
    La_cilm_complex[1, 2, 1] = a**2 * omega**2 / 30**.5 * (-m2*(1+m3))
    La_cilm_complex[0, 2, 2] = a**2 * omega**2 / (5**.5*24**.5) * (m2**2-m1**2)
    La_cilm_complex[1, 2, 2] = a**2 * omega**2 / (5**.5*24**.5) * (2*m1*m2)
