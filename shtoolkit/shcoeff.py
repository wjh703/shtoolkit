...
class SpharmCoeff():
    def __init__(
        ...
        if coeffs.ndim == 4:
            ...
            msg = f'Invalid ndim of coeffs {coeffs.ndim}, '+\
    ...
    def pole_tide_correct(self, reference_time: float = 2000.0):
        ...
        m1p, m2p = 0.0, 0.0
        for i in range(4):
            m1p += np.append(delta_time[dtime <= 2010.0] * ai_before[i], delta_time[dtime > 2010.0] * ai_after[i]).sum()
            m2p += np.append(delta_time[dtime <= 2010.0] * bi_before[i], delta_time[dtime > 2010.0] * bi_after[i]).sum()
    ...
    def __add__(self, other):
        if isinstance(other, SpharmCoeff):
            if (self.coeffs.shape == other.coeffs.shape and 
                    np.allclose(self.epochs, other.epochs, atol=0.5)):
